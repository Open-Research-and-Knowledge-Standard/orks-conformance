...
# Declared vendor roots for the Orca capability set. Both roots hold real
# tracked files, never symlinks: section 0 refuses a symlink anywhere in this
# repository and section 2 requires index mode 100644 for every path. The two
# roots therefore duplicate the same bytes deliberately, and section 8 is the
# control that refuses a silent divergence between them.
CAPABILITY_ROOTS=(
  .agents/skills
  .claude/skills
)
CAPABILITY_LOCK='skills-lock.json'
EXPECTED_CAPABILITIES=(
  ...
  orca-outcome-model
  orca-planning
  orca-project-profile
  orca-role-contract
  orca-session-start
  orca-verification
)
for capability_root in "${CAPABILITY_ROOTS[@]}"; do
  for capability in "${EXPECTED_CAPABILITIES[@]}"; do
    STATIC_PATHS+=("$capability_root/$capability/SKILL.md")
  done
done

# True for a path inside a declared vendor root. Every narrowed scan below
# routes through this one predicate so the exemption cannot widen by accident.
is_capability_path() {
  case "$1" in
    .agents/skills/*/SKILL.md|.claude/skills/*/SKILL.md) return 0 ;;
  esac
  return 1
}
...
EXPECTED_CAPABILITY_LOCK_SHA256='3dc79c2d2bc214f5bc28112a852024fcd5419ff788a6d89a74d703fe0ed6524d'
# Split so the literal never appears in this file: section 6 refuses a
# private-project indicator in repository-authored content, and this validator
# is repository-authored content.
EXPECTED_CAPABILITY_SOURCE="Probably""Computers/orca-baseline"
EXPECTED_CAPABILITY_SOURCE_TYPE='github'

# The vendored capability set is generated by a distribution this repository
# does not own and may not edit: editing a SKILL.md breaks the content hash
# that proves the copy unmodified. Seven of the fourteen carried non-ASCII text
# at baseline 0.23.0, so the project ASCII rule was relaxed for exactly those
# bytes, by file, by digest, by byte count, and by admitted code point.
#
# RETIRED 2026-08-19, at the baseline 0.24.0 update pass. That baseline compiles
# the capability set ASCII-clean - orca-baseline decisions/0117 - so the defect
# the exemption covered is gone. The stale-exemption control at the end of
# section 8 is what made this automatic: it failed on all seven entries the
# moment the ASCII-clean install landed, and the table was emptied in the same
# commit as the install.
#
# Empty is not absent. With no entry here, section 8 refuses ANY non-ASCII byte
# in ANY vendored SKILL.md under either root, which is the rule this repository
# holds everywhere else and now holds here too.
CAPABILITY_UNICODE_EXCEPTIONS=()
# The code points the retired exception admitted, as their exact UTF-8
# encodings: U+2014 EM DASH and U+2019 RIGHT SINGLE QUOTATION MARK. With the
# exception table empty, NOTHING is admitted anywhere in this repository - a
# capability carrying either of these now fails at the no-recorded-exception
# branch above and never reaches this list. It is kept, rather than emptied,
# because it is the second gate a future exception would have to pass and
# emptying it would leave that gate's `sed` with no expression.
CAPABILITY_ADMITTED_CODE_POINTS=(
  '\xe2\x80\x94'
  '\xe2\x80\x99'
)
...
# Git metadata must be present and must not be a symlink - a symlinked `.git`
# is the redirection this section exists to refuse. It may be either shape:
# a directory in a primary checkout, or a REGULAR FILE holding a `gitdir:`
# pointer in a linked worktree. Requiring a directory refused every Orca linked
# worktree, which is where CANON-007 tells a session to work, so this repository
# could only ever be validated on the canonical checkout. That was a portability
# defect, not a control: the pointer file is ordinary Git, the symlink refusal is
# unchanged, and every later section reads the index through `git` rather than
# through this path.
if [ -L "$REPO_ROOT/.git" ]; then
  fail 'Git metadata is symlinked'
  EARLY_SAFE=0
elif [ -d "$REPO_ROOT/.git" ]; then
  :
elif [ -f "$REPO_ROOT/.git" ]; then
  if ! LC_ALL=C grep -Eq '^gitdir: [^[:space:]]' "$REPO_ROOT/.git"; then
    fail 'Git worktree pointer file does not name a Git directory'
    EARLY_SAFE=0
  fi
else
  fail 'required repository directory is missing or symlinked'
...
# 120 -> 121: .gitignore for the 0.24.0 session-record obligation.
# Directory count does not move: the new file sits at the repository root.
  [ "${#EXPECTED_PATHS[@]}" -eq 121 ] || \
    fail 'approved repository inventory must contain exactly 121 files'
...
[ "${#EXPECTED_DIRECTORIES[@]}" -eq 49 ] || \
  fail 'approved repository inventory must contain exactly 49 directories'
...
for pin_spec in \
  "RULES.md:$EXPECTED_RULES_SHA256" \
...
for expected in "${EXPECTED_PATHS[@]}"; do
  ...
  case "$expected" in
    ...
    .agents/skills/*/SKILL.md|.claude/skills/*/SKILL.md)
      # Same treatment the manifest-pinned upstream JSON already receives, for
      # the same reason: the bytes are vendored and may not be edited here. The
      # ASCII rule is not waived, it is moved - section 8 pins each exempted
      # file by digest, by non-ASCII byte count, and by admitted code point.
      if ! LC_ALL=C tr -d '\000' < "$file_item" | cmp -s - "$file_item"; then
        fail 'NUL byte in a vendored capability file'
      fi
      if command -v iconv >/dev/null 2>&1 && \
        ! iconv -f UTF-8 -t UTF-8 "$file_item" >/dev/null 2>&1; then
        fail 'invalid UTF-8 in a vendored capability file'
      fi
      if ! tail -c 1 "$file_item" | cmp -s - <(printf '\n'); then
        fail 'vendored capability file does not end with a newline byte'
      fi
      ;;
    *)
...
while IFS= read -r -d '' markdown; do
  ...
  # The declared vendor roots are excluded from the link sweep and from
  # nothing else. Two of the fourteen capabilities carry a relative link into
  # the distribution's own tree, which resolves inside no consumer. That is a
  # defect in the distribution, recorded as such; it is not repairable here,
  # because editing a vendored file breaks the hash that proves it unmodified.
  find "$REPO_ROOT" \
    -path "$REPO_ROOT/.git" -prune -o \
    -path "$REPO_ROOT/.agents/skills" -prune -o \
    -path "$REPO_ROOT/.claude/skills" -prune -o \
...
for expected in "${EXPECTED_PATHS[@]}"; do
  ...
  case "$expected" in
    # There is no arm for `AGENTS.md` here. It carried one while `AGENTS.md` was
    # this repository's authored rules file and named `orks-planning`; the
    # generated entry point that replaced it names no private repository at all -
    # CANON-008 section 5.2b filters the chain and the generator implements it -
    # so the exemption protected nothing and is gone. `AGENTS.md` and `CLAUDE.md`
    # now fall to the default arm and are scanned like any other authored path,
    # which is the check that would catch a generator that stopped filtering.
    "$CAPABILITY_LOCK"|.agents/skills/*/SKILL.md|.claude/skills/*/SKILL.md)
      # The distribution's owner name is required provenance in these paths,
      # not leaked private-project authority: the lockfile must record where
      # every capability came from. The exemption is therefore paired with the
      # positive assertion in section 8 that the provenance is present and is
      # exactly the approved source. The private-payload pattern still applies.
      if LC_ALL=C grep -Eiq -- "$PRIVATE_PAYLOAD_PATTERN" "$file_item"; then
        fail 'private-payload indicator in a vendored capability path'
      fi
      ;;
    *)
...
printf '8. Vendored capability set\n'
CAPABILITY_LOCK_FILE="$REPO_ROOT/$CAPABILITY_LOCK"
if [ ! -f "$CAPABILITY_LOCK_FILE" ] || [ -L "$CAPABILITY_LOCK_FILE" ]; then
  fail 'capability lockfile is missing or is not a regular file'
else
  actual_capability_lock_sha256="$(sha256sum "$CAPABILITY_LOCK_FILE" | awk '{ print $1 }')"
  [ "$actual_capability_lock_sha256" = "$EXPECTED_CAPABILITY_LOCK_SHA256" ] || \
    fail 'capability lockfile differs from the approved cross-repository pin'

  # Exact lock accounting. Every assertion below is implied by the digest above
  # and is kept anyway, because a digest cannot say which fact moved and these
  # can. A widened install is the failure mode they exist for: it arrives with
  # a successful exit status and a lockfile that agrees with itself.
  lock_entry_count="$(grep -cE '^    "[^"]+": \{$' "$CAPABILITY_LOCK_FILE" || true)"
  [ "$lock_entry_count" -eq "${#EXPECTED_CAPABILITIES[@]}" ] || \
    fail 'capability lockfile does not record exactly the approved capability count'

  for capability in "${EXPECTED_CAPABILITIES[@]}"; do
    grep -Fqx "    \"$capability\": {" "$CAPABILITY_LOCK_FILE" || \
      fail 'capability lockfile omits an approved capability'
    grep -Fqx "      \"skillPath\": \"skills/$capability/SKILL.md\"," "$CAPABILITY_LOCK_FILE" || \
      fail 'capability lockfile omits an approved capability source path'
  done

  # Required provenance. The exemption granted in section 6 is only honest
  # because the owner name it tolerates is asserted here to be present, to be
  # the approved source, and to be a remote one.
  lock_source_count="$(grep -Fxc "      \"source\": \"$EXPECTED_CAPABILITY_SOURCE\"," "$CAPABILITY_LOCK_FILE" || true)"
  [ "$lock_source_count" -eq "${#EXPECTED_CAPABILITIES[@]}" ] || \
    fail 'capability lockfile does not record the approved provenance source for every capability'
  lock_source_type_count="$(grep -Fxc "      \"sourceType\": \"$EXPECTED_CAPABILITY_SOURCE_TYPE\"," "$CAPABILITY_LOCK_FILE" || true)"
  [ "$lock_source_type_count" -eq "${#EXPECTED_CAPABILITIES[@]}" ] || \
    fail 'capability lockfile does not record the approved provenance type for every capability'
  lock_hash_count="$(grep -cE '^      "computedHash": "[0-9a-f]{64}"$' "$CAPABILITY_LOCK_FILE" || true)"
  [ "$lock_hash_count" -eq "${#EXPECTED_CAPABILITIES[@]}" ] || \
    fail 'capability lockfile does not record a content hash for every capability'
  if grep -Eq '"source": "(/|~|\.\.?/)' "$CAPABILITY_LOCK_FILE"; then
    fail 'capability lockfile records a filesystem provenance path'
  fi
  if grep -Fq '"ref"' "$CAPABILITY_LOCK_FILE"; then
    fail 'capability lockfile records an unapproved source ref'
  fi
fi

for capability_root in "${CAPABILITY_ROOTS[@]}"; do
  for capability in "${EXPECTED_CAPABILITIES[@]}"; do
    capability_file="$REPO_ROOT/$capability_root/$capability/SKILL.md"
    if [ ! -f "$capability_file" ] || [ -L "$capability_file" ]; then
      fail 'approved capability is missing from a declared vendor root'
    fi
  done
done

for capability_root in "${CAPABILITY_ROOTS[@]}"; do
  [ -d "$REPO_ROOT/$capability_root" ] || continue
  while IFS= read -r -d '' capability_item; do
    relative="$(relative_path "$capability_item")"
    if ! is_capability_path "$relative"; then
      fail 'unexpected file in a declared vendor root'
      continue
    fi
    observed_capability="${relative#*/skills/}"
    observed_capability="${observed_capability%/SKILL.md}"
    observed_accounted=0
    for capability in "${EXPECTED_CAPABILITIES[@]}"; do
      [ "$observed_capability" != "$capability" ] || observed_accounted=1
    done
    [ "$observed_accounted" -eq 1 ] || \
      fail 'declared vendor root holds a capability the lockfile does not name'
  done < <(find "$REPO_ROOT/$capability_root" -type f -print0 | sort -z)
done

# Root agreement. The duplication across the two roots is deliberate and is
# enforced nowhere else: both roots are updated together or this goes red.
for capability in "${EXPECTED_CAPABILITIES[@]}"; do
  primary_capability_file="$REPO_ROOT/${CAPABILITY_ROOTS[0]}/$capability/SKILL.md"
  for root_index in "${!CAPABILITY_ROOTS[@]}"; do
    [ "$root_index" -ne 0 ] || continue
    mirror_capability_file="$REPO_ROOT/${CAPABILITY_ROOTS[$root_index]}/$capability/SKILL.md"
    [ -f "$primary_capability_file" ] && [ -f "$mirror_capability_file" ] || continue
    cmp -s "$primary_capability_file" "$mirror_capability_file" || \
      fail 'declared vendor roots hold different bytes for one capability'
  done
done

declare -A capability_unicode_pin=()
declare -A capability_unicode_bytes=()
for exception_spec in "${CAPABILITY_UNICODE_EXCEPTIONS[@]}"; do
  exception_name="${exception_spec%%:*}"
  exception_rest="${exception_spec#*:}"
  capability_unicode_pin["$exception_name"]="${exception_rest%%:*}"
  capability_unicode_bytes["$exception_name"]="${exception_rest#*:}"
done

STRIP_ADMITTED_CODE_POINTS=()
for admitted_code_point in "${CAPABILITY_ADMITTED_CODE_POINTS[@]}"; do
  STRIP_ADMITTED_CODE_POINTS+=(-e "s/$admitted_code_point//g")
done

for capability_root in "${CAPABILITY_ROOTS[@]}"; do
  for capability in "${EXPECTED_CAPABILITIES[@]}"; do
    capability_file="$REPO_ROOT/$capability_root/$capability/SKILL.md"
    [ -f "$capability_file" ] && [ ! -L "$capability_file" ] || continue
    non_ascii_bytes="$(LC_ALL=C tr -d '\011\012\040-\176' < "$capability_file" | wc -c)"
    [ "$non_ascii_bytes" -ne 0 ] || continue
    if [ -z "${capability_unicode_pin[$capability]+present}" ]; then
      fail 'vendored capability carries Unicode and is in no recorded exception'
      continue
    fi
    actual_capability_sha256="$(sha256sum "$capability_file" | awk '{ print $1 }')"
    [ "$actual_capability_sha256" = "${capability_unicode_pin[$capability]}" ] || \
      fail 'vendored capability has Unicode outside its recorded exception digest'
    [ "$non_ascii_bytes" -eq "${capability_unicode_bytes[$capability]}" ] || \
      fail 'vendored capability non-ASCII byte count differs from its recorded exception'
    remaining_code_points="$(
      LC_ALL=C sed "${STRIP_ADMITTED_CODE_POINTS[@]}" "$capability_file" |
        LC_ALL=C tr -d '\011\012\040-\176' | wc -c
    )"
    [ "$remaining_code_points" -eq 0 ] || \
      fail 'vendored capability carries a code point outside the admitted set'
  done
done

# An exemption that outlives the defect it was granted for is an exemption
# nobody notices is stale, so each one must still be needed.
for exception_spec in "${CAPABILITY_UNICODE_EXCEPTIONS[@]}"; do
  exception_name="${exception_spec%%:*}"
  exception_known=0
  for capability in "${EXPECTED_CAPABILITIES[@]}"; do
    [ "$exception_name" != "$capability" ] || exception_known=1
  done
  [ "$exception_known" -eq 1 ] || \
    fail 'Unicode exception names a capability that is not approved'
  exception_still_needed=0
  for capability_root in "${CAPABILITY_ROOTS[@]}"; do
    exception_file="$REPO_ROOT/$capability_root/$exception_name/SKILL.md"
    [ -f "$exception_file" ] && [ ! -L "$exception_file" ] || continue
    exception_bytes="$(LC_ALL=C tr -d '\011\012\040-\176' < "$exception_file" | wc -c)"
    [ "$exception_bytes" -eq 0 ] || exception_still_needed=1
  done
  [ "$exception_still_needed" -eq 1 ] || \
    fail 'Unicode exception is stale and must be retired'
done
